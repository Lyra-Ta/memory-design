...
test('chat 已有残缺配置：补默认并精简写回当前版', () => {
  ...
  assert.equal(cfg.timelineEnabled, true, '旧配置迁移后默认启用大总结时间轴化');
  assert.equal(cfg.summaryEnabled, true, '旧配置迁移后默认启用摘要到大总结');
  assert.equal('orchestration' in storedAt(deps), false);
});

test('v7 → v8：旧连接只归时间轴化，摘要默认跟随当前连接，旧内置模型提示升级', () => {
  const deps = mockVarDeps();
  ...
  assert.equal('connectionProfileId' in storedAt(deps), false, '写回后删除旧歧义字段');
});

test('功能启用开关：显式 false 原样保留并写回', () => {
  const deps = mockVarDeps();
  deps.store.chat[CONFIG_KEY] = {
  ...
  const cfg = loadConfig(deps);
  assert.equal(cfg.timelineEnabled, false);
  assert.equal(cfg.summaryEnabled, false);
  assert.equal(storedAt(deps).timelineEnabled, false);
  assert.equal(storedAt(deps).summaryEnabled, false);
});

test('普通总结配置：间隔最小 20，未知提示词 override 被丢弃', () => {
  const deps = mockVarDeps();
  deps.store.chat[CONFIG_KEY] = {
    version: CONFIG_VERSION,
    summaryInterval: 3,
    ...
    summaryOrchestrationOverrides: {
      pre: { content: '自定义普通总结前置', baseHash: 'old' },
  ...
  assert.deepEqual(Object.keys(cfg.summaryOrchestrationOverrides), ['pre']);
});
...
test('旧 global：清空会话状态、迁移自定义，并把 global 本身重写为精简形状', () => {
  const deps = mockVarDeps();
  const polluted = { ...legacyV4(), connectionProfileId: 'global-profile-id' };
  polluted.n = 350;
  ...
  assert.equal(cfg.timelineConnectionProfileId, 'global-profile-id');
  assert.equal(cfg.summaryConnectionProfileId, null);
...
test('saveGlobalDefault 只保存可继承设置与 override，不保存 chat 进度/内置全文', () => {
  ...
  cfg.summaryInterval = 80;
  cfg.summaryPlaceholderFloor = 445;
  cfg.summaryLastRemindedFloor = 440;
  cfg.summaryOrchestrationOverrides.post = { content: '普通总结自定义后置', baseHash: 'old-summary-post' };
  cfg.timelineEnabled = false;
  ...
  assert.equal(saved.summaryLastRemindedFloor, null);
  assert.equal(saved.summaryInterval, 80);
  assert.equal(saved.n, 250);
  assert.equal(saved.timelineConnectionProfileId, 'timeline-profile');
  assert.equal(saved.summaryConnectionProfileId, 'summary-profile');
  assert.equal(saved.modelHint, '保留这句');
  assert.equal(saved.timelineEnabled, false);
  assert.equal(saved.summaryEnabled, false);
  assert.equal('orchestration' in saved, false);
  assert.equal(
  ...
  assert.equal(
    ((saved.summaryOrchestrationOverrides as Record<string, { content: string }>).post).content,
    '普通总结自定义后置',
  );
});
...
test('v3 → 当前版：旧代理选择不冒充 Connection Profile，自定义提示词仍保留', () => {
  const deps = mockVarDeps();
  const old = legacyV4();
  deps.store.chat[CONFIG_KEY] = {
    ...old,
