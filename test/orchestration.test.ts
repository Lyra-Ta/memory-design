...
test('默认编排：后置思考要求与输出指令合并成一条', () => {
  const ids = defaultOrchestration().map(e => e.id);
  assert.deepEqual(ids, ['skeleton', 'historical_context', 'note', 'guidance', 'post']);
  const post = defaultOrchestration().find(e => e.id === 'post')!;
  ...
  const skeleton = defaultOrchestration().find(e => e.id === 'skeleton')!;
...
test('resolveOrchestration：只覆盖 content，结构元数据仍取当前内置版', () => {
  ...
  const resolved = resolveOrchestration({
    skeleton: { content: '用户版', baseHash: promptFingerprint(builtin.content) },
