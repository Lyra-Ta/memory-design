...
import { PROMPT_PREFERENCES_KEY } from '../src/plugin/prompt-preferences';
import type {
  ArchiverTavernDeps,
  GenerateRawArgs,
  GenerateRawResult,
...
import { defaultSummaryOrchestration, summaryPromptFingerprint } from '../src/plugin/summary-orchestration';
import {
  ArchiverSession,
...
test('generate：时间轴化只使用自己的 Connection Profile ID', async () => {
  ...
  assert.ok(deps.genCalls[0].ordered_prompts.every(prompt => prompt.role === 'system'));
  assert.doesNotMatch(
    deps.genCalls[0].ordered_prompts.map(prompt => prompt.content).join('\n'),
    /【(?:原始摘要|原始档案|在场档案) · 层 \d+】/,
  );
...
test('大总结调试 raw：保留本次整段输出；手改后表示当前候选全量', async () => {
  ...
  deps.genResult = { content: fullOutput, reasoning: '时间轴模型的独立 reasoning' };
  ...
  assert.equal(candidate.reasoning, '时间轴模型的独立 reasoning');
  const edited = session.editCandidate(candidate, candidate.body.replace('两人相遇并同行。', '两人相遇后结伴同行。'));
  assert.equal(edited.reasoning, candidate.reasoning, '编辑正文不能改写独立 reasoning');
  assert.match(edited.raw, /^<thinking>先整理时间线/);
  assert.match(edited.raw, /两人相遇后结伴同行/);
  assert.match(edited.raw, /模型尾注。$/);
});

test('大总结重新生成：用新响应的 reasoning 替换旧值', async () => {
  const deps = scene1();
  deps.genResult = { content: MERGED, reasoning: '首次 reasoning' };
  const session = new ArchiverSession(deps, defaultConfig());
  const first = await session.generate(session.refresh().table, '');

  deps.genResult = { content: MERGED, reasoning: '重生成 reasoning' };
  const rerolled = await session.regenerate(session.refresh().table, '', first.selection);

  assert.equal(first.reasoning, '首次 reasoning');
  assert.equal(rerolled.reasoning, '重生成 reasoning');
});

test('一键补正候选：只补机械闭合符并重新校验，不改变本轮范围', async () => {
  const deps = scene1();
  deps.genResult = {
    content: '<World_Archive>\n《合并档 | 1988年\n概览。',
    reasoning: '补正前的独立 reasoning',
  };
  const s = new ArchiverSession(deps, defaultConfig());
  ...
  assert.equal(repaired.candidate.reasoning, cand.reasoning);
...
test('提示词 override：首次编辑只写 global 新键，chat 配置保持无提示词', () => {
  const deps = scene1();
  const s = new ArchiverSession(deps, defaultConfig());
  ...
  assert.equal(
    s.promptPreferences.timelineOverrides.skeleton.acknowledgedBuiltinHash,
    promptFingerprint(builtin.content),
  );
  const globalStored = deps.vars.global[PROMPT_PREFERENCES_KEY] as Record<string, unknown>;
  assert.ok((globalStored.timelineOverrides as Record<string, unknown>).skeleton);
  assert.equal(PROMPT_PREFERENCES_KEY in deps.vars.chat, false);
  assert.equal(
    'orchestrationOverrides' in ((deps.vars.chat.memoryArchiver ?? {}) as Record<string, unknown>),
    false,
  );
  ...
  assert.equal(
    'skeleton' in ((deps.vars.global[PROMPT_PREFERENCES_KEY] as Record<string, unknown>).timelineOverrides as Record<string, unknown>),
    false,
    '使用内置后从 global 偏好删除 override',
  );
});

test('提示词新版三状态：查看零写入、继续自定义只确认、重新保存也确认当前版', () => {
  const deps = scene1();
  const s = new ArchiverSession(deps, defaultConfig());
  s.promptPreferences.timelineOverrides.skeleton = {
    content: '旧自定义',
    acknowledgedBuiltinHash: '旧内置指纹',
  };

  assert.equal(s.orchestrationState('skeleton').builtinUpdateAvailable, true);
  assert.equal(Object.keys(deps.vars.global).length, 0, '查看状态本身不写变量');

  s.acknowledgeOrchestrationBuiltin('skeleton');
  assert.equal(s.orchestrationEntries()[0].content, '旧自定义');
  assert.equal(s.orchestrationState('skeleton').builtinUpdateAvailable, false);

  s.promptPreferences.timelineOverrides.skeleton.acknowledgedBuiltinHash = '再次过期';
  s.setOrchestrationOverride('skeleton', '继续修改后的自定义');
  assert.equal(s.orchestrationState('skeleton').builtinUpdateAvailable, false);
  assert.equal(s.orchestrationEntries()[0].content, '继续修改后的自定义');
});
...
test('提示词 override：单项恢复与全部恢复立即切回内置版', () => {
  ...
  s.promptPreferences.timelineOverrides.skeleton = {
    content: '自定义前置',
    acknowledgedBuiltinHash: 'old-1',
  };
  s.promptPreferences.timelineOverrides.post = {
    content: '自定义后置',
    acknowledgedBuiltinHash: 'old-2',
  };
...
test('普通总结提示词：保存、确认新版、单项／全部恢复均只改 global 偏好', () => {
  const deps = summaryScene();
  const session = new ArchiverSession(deps, defaultConfig());
  const builtin = defaultSummaryOrchestration().find(entry => entry.id === 'post')!;

  session.setSummaryOrchestrationOverride('post', '我的总结后置');
  assert.equal(session.summaryOrchestrationState('post').customized, true);
  assert.equal(session.summaryOrchestrationState('post').builtinUpdateAvailable, false);
  assert.equal(
    session.promptPreferences.summaryOverrides.post?.acknowledgedBuiltinHash,
    summaryPromptFingerprint(builtin.content),
  );
  assert.ok(
    ((deps.vars.global[PROMPT_PREFERENCES_KEY] as Record<string, unknown>).summaryOverrides as Record<string, unknown>).post,
  );
  assert.equal(PROMPT_PREFERENCES_KEY in deps.vars.chat, false);

  session.setSummaryOrchestrationOverride('post', builtin.content);
  assert.equal(session.summaryOrchestrationState('post').customized, false, '保存成内置原文即删除 override');

  session.setSummaryOrchestrationOverride('post', '我的总结后置');
  session.promptPreferences.summaryOverrides.post!.acknowledgedBuiltinHash = '旧版';

  assert.equal(session.summaryOrchestrationState('post').builtinUpdateAvailable, true);
  session.acknowledgeSummaryOrchestrationBuiltin('post');
  assert.equal(session.summaryOrchestrationEntries().find(entry => entry.id === 'post')?.content, '我的总结后置');
  assert.equal(
    session.promptPreferences.summaryOverrides.post?.acknowledgedBuiltinHash,
    summaryPromptFingerprint(builtin.content),
  );
  ...
  session.setSummaryOrchestrationOverride('pre', '我的总结前置');
  session.resetSummaryOrchestrationOverride('post');
  assert.equal(session.summaryOrchestrationState('post').customized, false);
  assert.equal(session.summaryOrchestrationEntries().find(entry => entry.id === 'post')?.content, builtin.content);
  assert.deepEqual(session.summaryPromptOverrideSummary(), { customized: 1, updates: 0 });

  session.resetAllSummaryOrchestrationOverrides();
  assert.deepEqual(session.summaryPromptOverrideSummary(), { customized: 0, updates: 0 });
  assert.deepEqual(session.summaryOrchestrationEntries(), defaultSummaryOrchestration());
  assert.deepEqual(
    (deps.vars.global[PROMPT_PREFERENCES_KEY] as Record<string, unknown>).summaryOverrides,
    {},
  );
});

test('运行时槽由插件装配，不能被写成自定义提示词', () => {
  const deps = summaryScene();
  const session = new ArchiverSession(deps, defaultConfig());

  session.setOrchestrationOverride('historical_context', '不得覆盖时间轴运行时槽');
  session.setSummaryOrchestrationOverride('runtime', '不得覆盖总结运行时槽');

  assert.deepEqual(session.promptPreferences.timelineOverrides, {});
  assert.deepEqual(session.promptPreferences.summaryOverrides, {});
  assert.equal(PROMPT_PREFERENCES_KEY in deps.vars.global, false);
});
...
test('普通总结收集：给全部完整 World Archive，仅给 x 后完整 Flux', () => {
  ...
  assert.doesNotMatch(collected.archiveContext, /【(?:原始摘要|原始档案|在场档案) · 层 \d+】/);
  ...
  assert.doesNotMatch(collected.targetFlux, /【原始摘要 · 层 \d+】/);
...
test('普通总结主路：先创建末尾空白 y，应用后只把 y 写成无 marker Archive', async () => {
  ...
  assert.deepEqual(deps.genCalls[0].ordered_prompts.map(prompt => prompt.role), ['system', 'system', 'system']);
  ...
  assert.doesNotMatch(runtimePrompt, /【原始摘要 · 层 \d+】/);
  assert.doesNotMatch(runtimePrompt, /【(?:原始档案|在场档案) · 层 \d+】/);
  assert.deepEqual(candidate.prompts, deps.genCalls[0].ordered_prompts);
...
test('普通总结调试 raw：保留本次整段输出；手改后表示当前候选全量', async () => {
  ...
  deps.genResult = { content: fullOutput, reasoning: '普通总结模型的独立 reasoning' };
  const session = new ArchiverSession(deps, defaultConfig());
  const candidate = await session.generateSummary();

  assert.equal(candidate.raw, fullOutput);
  assert.equal(candidate.reasoning, '普通总结模型的独立 reasoning');
...
test('普通总结重新生成：用新响应的 reasoning 替换旧值', async () => {
  const deps = readySummaryScene();
  deps.genResult = { content: SUMMARY_RESULT, reasoning: '首次 reasoning' };
  const session = new ArchiverSession(deps, defaultConfig());
  const first = await session.generateSummary();

  deps.genResult = { content: SUMMARY_RESULT, reasoning: '重生成 reasoning' };
  const rerolled = await session.regenerateSummary(first, '');
...
test('普通总结失败重试：保持同一 y/来源/连接，只允许更换手动 guidance', async () => {
  ...
  session.promptPreferences.summaryOverrides.pre = {
    content: '中途修改不应污染已冻结轮次',
    acknowledgedBuiltinHash: 'old',
  };
