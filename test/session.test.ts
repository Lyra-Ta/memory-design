...
class MockDeps implements ArchiverTavernDeps {
  ...
  constructor(init: Record<number, string>, lastId: number) {
    for (const [k, v] of Object.entries(init)) this.floors.set(Number(k), v);
    this.lastId = lastId;
  }
  private one(id: number) {
    const m = this.floors.get(id);
    return m === undefined ? [] : [{ message_id: id, message: m }];
  }
...
test('collect：四份无 marker = 原始、无既存', () => {
  const s = new ArchiverSession(scene1(), defaultConfig());
  const c = s.collect(s.refresh().table);
...
test('范围选择：以最后勾选层为端点，强制纳入此前全部原始档', () => {
  ...
  const c = s.collect(table, [50, 150]); // 即使传入列表有洞，核心也按端点 150 连续收取
  assert.deepEqual(
...
test('generate：合法候选 → preview、through=200、带 Connection Profile ID', async () => {
  const deps = scene1();
  ...
  assert.equal(deps.genCalls[0].connection_profile_id, 'profile-1');
});
